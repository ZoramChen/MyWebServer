...
private:
    ...
    char *m_host;       //请求资源所在服务器的域名。
    long m_content_length;  //请求数据大小
    bool m_linger;   //连接管理类型  true--保持连接，响应后不关闭，可继续处理后续请求  flase--响应后关闭连接


    char *m_file_address;   //内存映射中读取服务器上的文件地址
    struct stat m_file_stat;
    struct iovec m_iv[2];
    int m_iv_count;
    int cgi;        //是否启用的POST（1-需要 0-不需要）
    char *m_string; //存储请求头数据
    int bytes_to_send;   //剩余发送字节数
    int bytes_have_send;  //已发送字节数
    char *doc_root;     //存放静态文件的根目录（绝对路径）
